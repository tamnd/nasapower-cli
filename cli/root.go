// Package cli assembles the nasapower command tree from the nasapower
...
// NewApp assembles the kit application from the nasapower domain. The
...
//
// To add a command, declare it in nasapower/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := nasapower.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(nasapower.Domain{}).Register(app)
