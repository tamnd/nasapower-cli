...
describes what users get. Pick a format with `-o`, or let nasapower choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nasapower <command> -o table   # aligned columns for reading
nasapower <command> -o jsonl   # one JSON object per line, for piping
nasapower <command> -o json    # a single JSON array
nasapower <command> -o csv     # spreadsheet friendly
nasapower <command> -o tsv     # tab-separated
nasapower <command> -o url     # just the URL column
nasapower <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nasapower <command> --fields id,title,url
```
...
```bash
nasapower <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nasapower <command>            # a table, because this is a terminal
nasapower <command> | wc -l    # JSONL, because this is a pipe
```
